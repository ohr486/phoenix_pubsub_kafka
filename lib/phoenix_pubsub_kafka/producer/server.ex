defmodule Phoenix.PubSub.Kafka.Producer.Server do
  ...
  use GenServer
  alias Phoenix.PubSub.Kafka.{Config, Topic}

  def start_link(name, opts) do
    Kafka.Logger.debug("Producer.Server.start_link(#{inspect name}, #{inspect opts})")
    GenServer.start_link(__MODULE__, [name, opts], name: name)
  end

  def init([name, opts]) do
    Kafka.Logger.debug("Producer.Server.init([#{inspect name}, #{inspect opts}])")
    Process.flag(:trap_exit, true)
    {:ok, %{}}
  end

  def broadcast(fastlane, pool_size, node_ref, from_pid, topic, payload) do
    Kafka.Logger.debug("Producer.Server.do_broadcast(#{inspect fastlane},#{inspect pool_size},#{inspect node_ref},#{inspect from_pid},#{inspect topic},#{inspect payload})")
    do_broadcast(fastlane, pool_size, node_ref, from_pid, topic, payload)
  end

  def direct_broadcast(arg) do
    # TODO: imple it
    Kafka.Logger.debug("TODO: imple Producer.Server.direct_broadcast(#{inspect arg})")
  end

  defp do_broadcast(fastlane, pool_size, node_ref, from_pid, phx_topic, payload) do
    Kafka.Logger.debug("Producer.Server.do_broadcast(#{inspect fastlane},#{inspect pool_size},#{inspect node_ref},#{inspect from_pid},#{inspect phx_topic},#{inspect payload})")
