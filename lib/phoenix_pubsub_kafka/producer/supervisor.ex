defmodule Phoenix.PubSub.Kafka.Producer.Supervisor do
  ...
  def start_link(name, opts) do
    Kafka.Logger.debug("Producer.Supervisor.start_link(#{inspect name}, #{inspect opts})")
  ...
  def init([server_name, opts]) do
    Kafka.Logger.debug("Producer.Supervisor.init([#{inspect server_name}, #{inspect opts}])")
    server_opts = opts
                  |> Keyword.merge(
                    name: server_name
                  )
